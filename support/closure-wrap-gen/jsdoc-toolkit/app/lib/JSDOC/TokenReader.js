                ...
                if (stream.look(0, false) == ":") {
                    if (string.indexOf(".") != -1) {
                        LOG.warn(
                            "The symbol '" +
                                string +
                                "' uses dot notation, but is written as a string literal."
                        );
                    }
                    tokens.push(
                        new JSDOC.Token(string.substr(1, string.length - 2), "NAME", "NAME")
                    );
                } else {
...
JSDOC.TokenReader.prototype.read_hex = function (/**JSDOC.TokenStream*/ stream, tokens) {
    ...
    while (!stream.look().eof) {
        if (JSDOC.Lang.isHexDec(found) && !JSDOC.Lang.isHexDec(found + stream.look())) {
            // done
